...
from .sim_data import compile_sim_data
...
def viz_model_preds(version,
                    ...
                    dataroot='/home/tanushri/Documents/Datasets/lss/nuscenes/v1.0-test/',
                    map_folder='/home/tanushri/Documents/Datasets/lss/nuscenes/v1.0-test/',
                    gpuid=0,
                    viz_train=False,
...
def multi_viz_model_preds(version,
                    modelf,
                    dataroot='/home/tanushri/Documents/Datasets/nuscenes/v1.0-test/',
    ...
    trainloader, valloader = compile_data(version, dataroot, data_aug_conf=data_aug_conf,
                                          grid_conf=grid_conf, bsz=bsz, nworkers=nworkers,
                                          parser_name='multisegmentationdata', map_folder=map_folder)
    loader = trainloader if viz_train else valloader
    nusc_maps = get_nusc_maps(map_folder)
    ...
    with torch.no_grad():
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binmaps) in enumerate(loader):
            ...
            for si in range(imgs.shape[0]):
                ...
                for i in range (0,3):
                    ...
                    ax.get_yaxis().set_ticks([])
                    plt.setp(ax.spines.values(), color='b', linewidth=0)
                    plt.imshow(out[si,i], vmin=0, vmax=1, cmap='Blues')
                    ...
                    print('saving', imname)
                    if i == 0:
                        plt.savefig("outputMulti2/car/"+imname)
                    if i == 1:
                        plt.savefig("outputMulti2/lane/"+imname)
                    if i == 2:
                        plt.savefig("outputMulti2/road/"+imname)
        ...
        if video_output:
            for i in range (0,3):
                if i == 0:
                    generate_video_from_imgs('outputMulti2/car/', '.jpg')
                if i == 1:
                    generate_video_from_imgs('outputMulti2/lane/', '.jpg')
                if i == 2:
                    generate_video_from_imgs('outputMulti2/road/', '.jpg')

def sim_preds(modelf):
    dataroot='/home/tanushri/Documents/Datasets/nuscenes/v1.0-test/',
    gpuid=0,
    viz_train=False,
    video_output=True,
    ...
    loader = compile_sim_data(dataroot, data_aug_conf=data_aug_conf, grid_conf=grid_conf, bsz=bsz, nworkers=nworkers)
