...
def sim_lss_depth(version,
    ...
    trainloader, valloader = compile_depth_data(version, dataroot, data_aug_conf=data_aug_conf,
                                          grid_conf=grid_conf, bsz=bsz, nworkers=nworkers)

    device = torch.device('cpu') if gpuid < 0 else torch.device(f'cuda:{gpuid}')

    model = compile_depth_model(grid_conf, data_aug_conf)
    if modelf is not None:
        model.load_state_dict(torch.load(modelf))
    model.to(device)
    ...
    #Change to reprojection loss/ simple loss might also work
    if gpuid < 0:
        loss_fn = SimpleLoss(pos_weight).cpu()
    else:
        loss_fn = SimpleLoss(pos_weight).cuda(gpuid)
